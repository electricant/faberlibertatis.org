...
class Pico_Editor {
	...
	public function __construct()
	{
		...
		if(file_exists($this->plugin_path .'/config_pico_editor.php')){
			global $pico_editor_password;
			include_once($this->plugin_path .'/config_pico_editor.php');
	...
	private function do_new()
	{
		...
		if(!$file) die(json_encode(array('error' => 'Error: Invalid file name: '. basename($title))));
...
*/';
	...
	private function do_open()
	{
		...
		$file = strip_tags($file_url);
		if(!$file) die('Error: Invalid filename');
		
		$file .= CONTENT_EXT;
		if(file_exists(CONTENT_DIR . $file)) die(file_get_contents(CONTENT_DIR . $file));
		else die('Error: Does the file ' . $file . ' exist?');
	}
